    ...
    if request.method == "POST":
        try:
            ...
            if Profile.objects.filter(username=username).exists():
                return JsonResponse(
                    {"status": 400, "message": "用戶名已存在"}, status=400
                )
...
def get_information(request):
    if request.method == "GET":
        ...
        if not userId:
            return JsonResponse(
                {"status": 400, "message": "缺少用戶ID參數"}, status=400
            )
        ...
        try:
            ...
            return JsonResponse(
                {"status": "error", "message": "用戶不存在"}, status=404
            )
        ...
        # return HttpResponseNotAllowed(['POST'])
        return JsonResponse(
            {"status": 400, "success": False, "message": "只接受GET請求"}, status=400
        )
