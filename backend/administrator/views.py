...
from backend.user.decorators import login_required
...
def addAdministrator(request):
    try:
        ...
        if Administrator.objects.filter(email=data["email"]).exists():
            return JsonResponse(
                {"message": "此電子郵件已被其他管理者使用", "adminId": None}, status=400
            )
        if Administrator.objects.filter(admin_name=data["adminName"]).exists():
            return JsonResponse(
                {"message": "此名稱已被其他管理者使用", "adminId": None}, status=400
            )
        ...
        return JsonResponse(
            {"message": "Profile not found.", "adminId": None}, status=404
        )
...
def deleteAdministrator(request):
    try:
        ...
        return JsonResponse(
            {"message": "Administrator deleted successfully."}, status=200
        )
...
def deleteCafe(request):
    try:
        ...
        cafe.delete()
        return JsonResponse(
            {"message": "Cafe deleted successfully.", "success": True}, status=200
        )

    except Cafe.DoesNotExist:
        return JsonResponse(
            {"message": "Cafe not found.", "success": False}, status=404
        )
    except json.JSONDecodeError:
...
def judgeCafe(request):
    try:
        ...
        cafe.save()
        return JsonResponse(
            {"message": "Cafe updated successfully.", "success": True}, status=200
        )
    ...
    try:
        ...
        for cafe in cafe_list:
            cafe_list_json.append(
                {
                    "cafeId": str(cafe.cafe_id),
                    "cafeName": cafe.cafe_name,
                    "isLegal": cafe.legal,
                    "userId": str(cafe.owner.uid),
                }
            )
...
def deleteUser(request):
    try:
        ...
        user_profile.delete()
        return JsonResponse(
            {"message": "User deleted successfully.", "success": True}, status=200
        )
...
def add_new_cafe(request):
    if request.method == "POST":
        ...
        try:
            ...
            cafe.save()
            return JsonResponse(
                {"status": 200, "success": True, "cafe_info": cafe_info}, status=200
            )

        except Exception as e:
            return JsonResponse(
                {"status": 500, "success": False, "message": str(e)}, status=500
            )

    else:
        # return HttpResponseNotAllowed(['POST'])
        return JsonResponse(
            {"status": 400, "success": False, "message": "只接受 POST 請求"}, status=400
        )
...
def delete_cafe(request):
    if request.method == "POST":
        ...
        try:
            ...
            cafe.delete()
            return JsonResponse(
                {"status": 200, "success": True, "message": "咖啡廳刪除成功"},
                status=200,
            )

        except Cafe.DoesNotExist:
            return JsonResponse(
                {"status": 404, "success": False, "message": "咖啡廳不存在"}, status=404
            )
    ...
    return JsonResponse(
        {"status": 400, "success": False, "message": "只接受POST請求"}, status=400
    )
